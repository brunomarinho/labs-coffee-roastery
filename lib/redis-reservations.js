...
const CONFIRM_PURCHASE_SCRIPT = `
  ...
  if quantity > 0 then
    ...
    local newInventory = redis.call('decrby', inventoryKey, quantity)
...
export async function reserveInventory(inventoryId, sessionId, quantity = 1, clientIP = null) {
  ...
  if (!redis) {
    console.error('Redis not available for inventory reservation');
  ...
  try {
    ...
    if (currentReserved && parseInt(currentReserved) >= MAX_RESERVATIONS_PER_ITEM) {
      console.log(`Too many reservations for ${inventoryId}: ${currentReserved}`);
    ...
    if (clientIP) {
      ...
      if (ipReservations && parseInt(ipReservations) >= MAX_RESERVATIONS_PER_IP) {
        console.log(`Too many concurrent reservations for IP ${clientIP}`);
    ...
    if (result === 1) {
      ...
      console.log(`Reserved ${quantity} units of ${inventoryId} for session ${sessionId}`);
      return { success: true };
    } else {
      console.log(`Failed to reserve ${inventoryId} - insufficient inventory`);
...
export async function releaseReservation(inventoryId, sessionId) {
  if (!redis) {
    console.error('Redis not available for releasing reservation');
    return 0;
  }
  ...
  try {
    ...
    if (result > 0) {
      console.log(`Released ${result} units of ${inventoryId} from session ${sessionId}`);
    }
...
export async function confirmPurchase(inventoryId, sessionId) {
  if (!redis) {
    console.error('Redis not available for confirming purchase');
    return 0;
  }
  ...
  try {
    ...
    if (result > 0) {
      console.log(`Confirmed purchase of ${result} units of ${inventoryId} for session ${sessionId}`);
    }
...
export async function getAvailableInventory(inventoryId) {
  if (!redis) {
    console.warn('Redis not available - returning 0 availability');
    return 0;
  }
...
export async function cleanupOrphanedReservations() {
  if (!redis) {
    console.error('Redis not available for cleanup');
    return 0;
  }
  ...
  try {
    ...
    for (const key of reservationKeys) {
      ...
      if (ttl === -1) {
        ...
        console.log(`Cleaned orphaned reservation: ${sessionId}`);
      }
    }
    ...
    for (const reservedKey of reservedKeys) {
      ...
      if (ttl === -1) {
        ...
        console.log(`Reset orphaned reserved counter: ${reservedKey}`);
      }
    }
